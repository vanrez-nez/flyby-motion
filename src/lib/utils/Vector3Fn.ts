...
/**
 * Transforms the vec3 with a mat4 (implicit w=1)
 */
...
/**
 * Same as transformMat4 but without translation (scale+rotate only)
 */
export function scaleRotateMat4(out: number[], a: number[], m: number[] | Float32Array): number[] {
    const x = a[0];
    const y = a[1];
    const z = a[2];
    let w = (m[3] * x + m[7] * y + m[11] * z) || 1.0;
 ...
 */
export function transformQuat(out: number[], a: number[], q: number[] | Float32Array): number[] {
